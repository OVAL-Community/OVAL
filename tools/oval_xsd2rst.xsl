...
<xsl:stylesheet version="2.0"
  ...
  <xsl:variable name="oval-var_namespace_prefix">oval-var</xsl:variable>
    ...
    <xsl:variable name="rst_h1"><xsl:text>=========================================================&#xa;</xsl:text></xsl:variable>
    <xsl:variable name="rst_h2"><xsl:text>---------------------------------------------------------&#xa;</xsl:text></xsl:variable>
	<xsl:variable name="rst_h3"><xsl:text>^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^&#xa;</xsl:text></xsl:variable>
	<xsl:variable name="rst_h4"><xsl:text>"""""""""""""""""""""""""""""""""""""""""""""""""""""""""&#xa;</xsl:text></xsl:variable>
      
	<xsl:template match="xsd:schema">
		<xsl:for-each select="xsd:annotation">
		    <xsl:text>Open Vulnerability and Assessment Language: </xsl:text><xsl:value-of select="xsd:appinfo/schema"/><xsl:value-of select="$md_br" />
		    <xsl:value-of select="$rst_h1" />
		...
		<xsl:if test="xsd:element[contains(@name, '_test')]">
		    <xsl:text>Test Listing</xsl:text><xsl:value-of select="$md_br" />
		...
		<xsl:if test="xsd:element[contains(@name, '_item')]">
	...
	<xsl:template name="attributes">
		...
		<xsl:text>.. list-table:: Attributes</xsl:text><xsl:value-of select="$md_br" />
		<xsl:text>    :header-rows: 1</xsl:text>
		<xsl:value-of select="$md_br" />
		<xsl:value-of select="$md_br" />
		<xsl:text>    * - Attribute</xsl:text><xsl:value-of select="$md_br" />
		<xsl:text>      - Type</xsl:text><xsl:value-of select="$md_br" />
		<xsl:text>      - Desc.</xsl:text><xsl:value-of select="$md_br" />
	...
	<xsl:template name="children">
	    ...
	    <xsl:text>.. list-table:: Elements</xsl:text><xsl:value-of select="$md_br" />
 	    <xsl:text>    :header-rows: 1</xsl:text>
		<xsl:value-of select="$md_br" />
		<xsl:value-of select="$md_br" />
		<xsl:text>    * - Child Elements</xsl:text><xsl:value-of select="$md_br" />
		<xsl:text>      - Type (MinOccurs..MaxOccurs)</xsl:text><xsl:value-of select="$md_br" />
		<xsl:text>      - Desc.</xsl:text><xsl:value-of select="$md_br" />
	...
	<xsl:template name="enumeration">
		<xsl:text>.. list-table:: Enumeration Values</xsl:text><xsl:value-of select="$md_br" />
		<xsl:text>    :header-rows: 1</xsl:text>
		<xsl:value-of select="$md_br" />
		<xsl:value-of select="$md_br" />
		<xsl:text>    * - Value</xsl:text><xsl:value-of select="$md_br" />
		<xsl:text>      - Description</xsl:text><xsl:value-of select="$md_br" />
	...
	<xsl:template name="deprecation_info">
		...
		<xsl:if test="$depInfo/oval:comment">
			<xsl:text>* Comment: </xsl:text><xsl:value-of select="$depInfo/oval:comment"/><xsl:value-of select="$md_br"/>
		</xsl:if>
		<xsl:value-of select="$md_br" />
	</xsl:template>
	...
	<xsl:template name="element_header">
		...
		<xsl:text>.. _</xsl:text><xsl:value-of select="$el/@name"/><xsl:text>:</xsl:text>
		<xsl:value-of select="$md_br" />
		<xsl:value-of select="$md_br" />
		<xsl:value-of select="$prefix"/><xsl:text> </xsl:text> <xsl:value-of select="$el/@name"/> <xsl:text> </xsl:text><xsl:value-of select="$suffix"/>
		<xsl:if test="$el/xsd:annotation/xsd:appinfo/oval:deprecated_info">
			<xsl:text> (Deprecated)</xsl:text>
		</xsl:if>
	    <xsl:value-of select="$md_br" />
	    <xsl:value-of select="$rst_h2" />
